...
workflow QC_ILLUMINA {   
    ...
    main:
        ...
        CSVTK_CONCAT_STATS_INPUT(SEQKIT_STATS_INPUT_ILLUMINA.out.stats.map { cfg, stats -> stats }.collect().map { files -> tuple([id: "illumina_reads.input_seqstats"], files)}, in_format, out_format )
        ...
        SEQKIT_STATS_QC_ILLUMINA(qc_reads)
        CSVTK_CONCAT_STATS_QC(SEQKIT_STATS_QC_ILLUMINA.out.stats.map { cfg, stats -> stats }.collect().map { files -> tuple([id: "illumina_reads.qc_seqstats"], files)}, in_format, out_format )
    
    emit:
        qc_reads
        input_stats = SEQKIT_STATS_INPUT_ILLUMINA.out.stats
        qc_stats = SEQKIT_STATS_QC_ILLUMINA.out.stats
