...
def main():
    ...
    parser.add_argument(
        ...
        help="Output file for individual assemblies (default: assemblies_with_genome_size.csv)"
    )
    parser.add_argument(
        "-o2", "--output_organisms",
        default="organism_average_genome_size.tsv",
        help="Output file for average genome size per organism (default: organism_average_genome_size.csv)"
    )
    ...
    # Format genome size with commas
    # output_df.loc[:, 'genome_size'] = output_df['genome_size'].round(0).astype(int).map('{:,}'.format)

    # Save individual assembly records
    ...
    remaining_df = others_df[~others_df["species_taxid"].isin(
        species_with_complete)]
    grouped_other = (
    ...
    grouped_df = pd.concat(
        [grouped_complete, grouped_other], ignore_index=True)
    grouped_df["avg_genome_size"] = grouped_df["avg_genome_size"].round(
        0).astype(int)

    # Format genome size with commas
    # grouped_df["avg_genome_size"] = grouped_df["avg_genome_size"].map('{:,}'.format)
    ...
    print(
        f"Saved organism-level average genome sizes to: {args.output_organisms}")
